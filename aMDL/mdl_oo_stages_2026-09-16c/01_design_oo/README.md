# 01_design_oo — MDL design, object-oriented (2026-09-16, mdl 2026-09-16.03)
...
stage, exactly like `02_Sequential_RT_Zemax_Validation_OOP` next to `02_validation_zemax`.

    01_design_oo\
        tradeoff_gui.py          STAGE 0: design-space GUI (Tkinter) -- any
                                 aperture in mm or inch, NA or F-number,
                                 band, H; feasibility verdict, pair maps,
                                 (D, H) sweeps (paper Fig. 1b/c/d, Eq. 7)
        tradeoff_maps.py         the same from the command line (check | study)
        tradeoff\                LensSpec / Ceiling / Feasibility (space.py),
                                 PairMapStudy / SweepStudy + presets (study.py),
                                 TradeoffApp (gui.py)
...
## Run
    ...
    python 01_design_oo\tradeoff_gui.py            # design space first
...
`config.json` keeps the key set of the old driver, so `02_Rayleigh_Sommerfeld_Validation_OOP`,
`02_Sequential_RT_Zemax_Validation_OOP` and `03_tapeout_oo` consume the run folders
...
was missing. The "next" hint points at `02_Rayleigh_Sommerfeld_Validation_OOP/run_verify.py`.
