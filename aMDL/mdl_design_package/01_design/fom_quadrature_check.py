...
written. Usage (from the package root):
    ...
    python 01_design\\fom_quadrature_check.py runs\\<run_folder> [other_m.npy]
...
both rules, and the midpoint J against design_metrics.json's J_final
(they must agree to float32 precision -- this is the check that the
rebuilt problem IS the run's problem). Read: the sinc/midpoint ratio
...
from mdl_core import MDLProblem                       # noqa: E402
...
def main():
    ...
    if j_final is not None:
        d = J["midpoint"] - float(j_final)
        print("  design_metrics.json J_final = %.4f -> midpoint rebuild %s "
              "(diff %.1e)" % (j_final, "MATCHES" if abs(d) < 2e-4 else
                               "DIFFERS: rebuilt problem != run's problem",
                               d))
