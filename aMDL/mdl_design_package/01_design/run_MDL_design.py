...
S3_CONTINUOUS = {
    ...
    "fom_mode": "geomean",           # "mean": J=<I>_w (paper Eq. 2; may
                                     ...
                                     # "softmin": smooth worst-case
                                     #   (log-sum-exp minimax; grating-
                                     #   coupler broadband inverse-design
                                     #   objective) -- maximizes the WORST
                                     #   wavelength; anneal beta below
    "softmin_beta": 20.0,            # softmin only: initial sharpness
                                     #   (beta*I ~ few => still mean-like)
    "softmin_beta_final": 300.0,     # softmin only: beta annealed to this
                                     #   during the gradient stage
                                     #   (None = keep softmin_beta fixed)
    "overlap_fom": False,            # True: grating-coupler-style mode-
                                     #   overlap objective (encircled
                                     #   energy in a focal disc, per-lam
                                     #   normalized to the ideal lens) --
                                     #   penalizes halo/satellite energy
                                     #   by construction. Memory ~ Nw *
                                     #   overlap_n_r0 * N_rings complex64:
                                     #   fine for combs, heavy for dense
                                     #   continuous bands.
    "overlap_r_enc_um": None,        # None = CHROMATIC disc,
                                     #   r_enc(lam) = overlap_airy_factor
                                     #   * 0.61 lam/NA per wavelength
                                     #   (same shape in Airy units at
                                     #   every line); a number forces one
                                     #   fixed disc -- NOT recommended: a
                                     #   lam_max-sized disc lets blue
                                     #   lines pass with halo (measured
                                     #   eta 0.18 vs true eff 0.004 at
                                     #   400 nm) and tolerates ~r_enc/NA
                                     #   of axial elongation
    ...
    "efficiency_corr_npz": None,     # path to a rigorous/scalar relative
                                     ...
                                     #   mdl_core_Dev_v1.relative_correction_table
                                     #   fed by a Lumerical RCWA / torcwa
                                     #   sweep of zone_table.npz); None =
                                     #   bare scalar model
    # -- fabrication ------------------------------------------------------
    "ring_width_um": 2.0,
    "h_max_um": 15.0,
    "dh_um": 0.078,
    # -- optimizer --------------------------------------------------------
    "seed_mode": "harmonic",      # "harmonic": best (lam0, p) harmonic
                                  #   seed from the seed_lam0_um grid
                                  # "echelle": analytic blaze-chart fold
                                  #   selection over ALL target lines
                                  #   (comb mode only; candidate folds
                                  #   judged by the live fom_mode;
                                  #   ignores seed_lam0_um)
    ...
    "verify_r_points": 601,
    # I(r,z) tile maps (fig_rz_tiles; defaults = paper Fig. 2e window).
    # run_verify.py falls back to these same values via cfg.get() only
    # for run folders created before these keys existed.
    "rzmap_r_max_um": 20.0,       # tile half-width in r (mirrored)
    "rzmap_r_points": 41,
    "rzmap_z_span_um": 1000.0,    # tile z-window: F +/- span
    "rzmap_z_points": 121,
    # -- packaging --------------------------------------------------------
...
# grating-coupler-style engine demo on the paper's 14-line comb:
# echelle-selected fold seed + softmin (smooth worst-case) objective +
# mode-overlap FOM. This is the direct attack on the mean-vs-locking
# triangle: softmin defends every line like geomean but without
# sinking to a low common denominator, and the overlap objective
# charges halo / satellite-focus energy to the FOM by construction.
S3_COMB_SOFTMIN = dict(S3_COMB,
                       name="s3_comb_softmin_overlap",
                       fom_mode="softmin",
                       seed_mode="echelle",
                       overlap_fom=True,
                       dll_file_no=6)
...
def main(cfg):
    ...
    if prob.fom_mode == "softmin":
        prob.softmin_beta = float(cfg.get("softmin_beta", 20.0))
    ...
    if cfg.get("efficiency_corr_npz"):
        lam_c, r_c, corr_c = load_efficiency_table(
            cfg["efficiency_corr_npz"])
        prob.apply_efficiency(lam_c, r_c, corr_c)
        log("rigorous efficiency correction applied from %s "
            "(%d wavelengths x %d zones, ratio %.2f-%.2f)"
    ...
    if cfg.get("overlap_fom"):
        ...
        if cfg.get("overlap_r_enc_um") is None:
            log("overlap objective: CHROMATIC disc r_enc(lam) = "
                "%.1f Airy radii = %.2f-%.2f um across the band "
                "(%d radial nodes, kernel table %.0f MB)"
                % (cfg.get("overlap_airy_factor", 2.0),
                   prob.r_enc.min(), prob.r_enc.max(),
                   prob._enc_r0.shape[1], prob.K.nbytes / 1e6))
        else:
            log("overlap objective: FIXED disc r <= %.2f um "
                "(%d radial nodes, kernel table %.0f MB)"
                % (prob.r_enc[0], prob._enc_r0.shape[1],
                   prob.K.nbytes / 1e6))
    log("problem: N=%d rings, M=%d levels, fom_mode=%s, objective=%s"
    ...
    if cfg["reuse_design_npy"]:
        ...
        if seed_mode == "echelle":
            if not comb_mode:
                raise SystemExit("seed_mode 'echelle' needs a discrete "
                                 ...
                                 "chart is built on the target lines)")
            m_seed, einfo = seed_echelle(prob, np.asarray(lams, float),
                                         verbose=log)
            best = (m_seed, prob.fom(m_seed),
                    ("echelle", einfo["h_fold_um"]))
            seed_record = {"mode": "echelle",
                           "h_fold_um": einfo["h_fold_um"],
                           "orders": [int(o) for o in einfo["orders"]],
                           "blaze_score": einfo["score"]}
            log("echelle seed: H_fold=%.4f um  J=%.4f"
                % (einfo["h_fold_um"], best[1]))
        else:
            best = (None, -1.0, None)
            for lam0 in cfg["seed_lam0_um"]:
                n0 = float(prob.n_func(lam0))
                pmax = max(1, int(np.floor(cfg["h_max_um"] * (n0 - 1.0)
                                           / lam0)))
                for p in sorted({1, 2, 4, 8, pmax // 2, pmax}):
                    ...
                    if f > best[1]:
                        best = (m, f, (lam0, p))
            seed_record = {"mode": "harmonic", "lam0_um": best[2][0],
                           "p": best[2][1]}
        ...
        beta_final = (cfg.get("softmin_beta_final")
                      if prob.fom_mode == "softmin" else None)
        m_g, f_g = gradient_refine(prob, m_sm,
                                   iters=cfg["gradient_iters"],
                                   softmin_beta_final=beta_final)
        log("Gradient: J=%.4f%s"
            % (f_g, ("  (softmin beta annealed to %.0f)"
                     % prob.softmin_beta) if beta_final else ""))
        m_f, f_f = hooke_jeeves(prob, m_g, d0=cfg["hja_d0"],
                                max_sweeps=cfg["hja_max_sweeps"])
        log("Polish: J=%.4f" % f_f)
        stages = {"J_seed": best[1], "seed": seed_record,
    ...
    json.dump(stages, open(os.path.join(run_dir, "design_metrics.json"),
                           "w"), indent=1)
    # local-grating decomposition for an external rigorous (RCWA) sweep
    zt_lams = np.asarray(lams if comb_mode
                         else cfg["verify_wavelengths_um"], float)
    zones = extract_local_gratings(prob, np.asarray(m_f, np.int32))
    write_zone_table(os.path.join(run_dir, "zone_table.npz"), zones,
                     zt_lams, prob)
    log("zone_table.npz: %d local-grating zones (outermost period "
        "%.1f um) -- sweep with a rigorous solver (Lumerical RCWA / "
        "torcwa), form corr = eta_rcwa/eta_tea with "
        "mdl_core_Dev_v1.relative_correction_table, then redesign with "
        "efficiency_corr_npz pointing at the correction file"
        % (len(zones), zones[-1]["period_um"]))
    log("saved: m_final.npy, %s, design_metrics.json, zone_table.npz"
        % ring_name)
