"""Regression of 02_validation_rs_oo against the legacy 02_validation_rs
...
re-exported ring table, bit for bit.
    ...
    python 02_validation_rs_oo\\tests\\test_against_legacy.py runs\\<run> [runs\\<run2> ...]
...
NEW = os.path.join(PKG_ROOT, "02_validation_rs_oo")
...
def main(run_dirs: List[str]) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="rsval_reg_") as tmp:
        for rd in run_dirs:
            ...
            print("%s: running 02_validation_rs_oo ..." % name, flush=True)
