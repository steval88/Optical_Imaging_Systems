"""
run_MDL_design.py -- unified MDL design driver (replaces run_design.py
and run_s3.py).

All settings live in ONE dictionary. Edit `SETTINGS = ...` at the top of
the CONFIG section and run:

    python run_MDL_design.py

Every run creates its own timestamped folder
...
the code and settings that produced it. Downstream:

    python run_verify.py  runs/<folder>      (RS diffraction check)
    python make_plots.py  runs/<folder>      (figures)
...
from aMDL.old.mdl_design_package.mdl_core import (MDLProblem, PAPER_COMB_14, multistep_GA_HJA_combo,
...
S3_CONTINUOUS = {
    ...
    "runs_dir": "runs",
    "snapshot_scripts": ["run_MDL_design.py", "mdl_core.py",
                         "run_verify.py", "make_plots.py",
                         "export_gds.py", "mdl_zemax_validation.py"],
}
...
SETTINGS = S3_COMB #S3_CONTINUOUS          # <-- EDIT: pick / customize a dictionary
...
def main(cfg):
    ...
    run_dir = os.path.join(cfg["runs_dir"], "%s_%s" % (stamp, cfg["name"]))
    scripts_dir = os.path.join(run_dir, "scripts")
    os.makedirs(scripts_dir)
    for fn in cfg["snapshot_scripts"]:
        if os.path.exists(fn):
            shutil.copy2(fn, os.path.join(scripts_dir, fn))
    ...
    log("done -> %s" % run_dir)
    log("next:  python run_verify.py %s" % run_dir)
    return run_dir
