...
# BPM wavelengths: config override, else every 3rd verification lambda
# (BPM cost ~ NR x NZ per wavelength; keep the subset small)
LAMS = cfg.get("bpm_wavelengths_um",
               list(np.asarray(cfg["verify_wavelengths_um"])[::3]))
log("wavelengths: %s um" % ", ".join("%.2f" % l for l in LAMS))
